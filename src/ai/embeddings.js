  ...
  // embedding validae kore: like dimensions,number ki na.
  function validateEmbedding(response) {
    const embedding = response.embeddings?.[0]?.values; //only first er ta.
    ...
    if(!embedding.every(Number.isFinite)){ //same Nan kina
   ...
   //actual  embedding generate korbe.
  async function generateEmbedding(text) {
    ...
    for(let i=1;i<=maxAttempts;i += 1) {
      try {
        ...
        if (!isRetryableError(error) || i === maxAttempts){//track max attems end or {
          throw error;
        }

        const delay = 1000 * 2 ** (i - 1);
        console.warn(
          `Embedding request failed temporarily. Retrying in ${delay}ms (${i}/${maxAttempts}).`
        );
        await wait(delay);
      }
    }
